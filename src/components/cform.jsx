...
import {  Box , Button , Grid , TextField ,Tooltip, MenuItem , Select  ,FormControl , InputLabel } from '@mui/material';
...
const Cform = (props) => {
  ...
  const handleSubmit = (event) => {
    ...
    .then((res) => {
      ...
      if(res.data[0].toString()=="true")
      res.data[0].toString()=="true"?props.history.push(
        {pathname:"/complaintt",state:[res.data[1]],})
    ...
    return ( 
        <div style={{marginLeft:"auto",marginRight:"auto"}}>
                ...
                <TextField 
                 inputProps={ariaLabel} fullWidth
                  variant="filled" required 
                   ...
                   onChange={handleChange} />
                    {errors && (
                <Typography variant="caption">{errors.productModelNumber}</Typography>
              )}
                </Box>
            <Box mb={3}>
                <TextField  
                inputProps={ariaLabel} fullWidth 
                variant="filled" 
                 label="ComplaintName"required
                 value={user.complaintName}
                 name="complaintName"
                 onChange={handleChange}/>
                  {errors && (
                <Typography variant="caption">{errors.complaintName}</Typography>
              )}
                </Box>
              ...
              {errors && <Typography variant="caption">{errors.status}</Typography>}
             </Box>
                <Box mb={3}>
                <TextField 
                 inputProps={ariaLabel} fullWidth 
                 variant="filled"  label="ClientId" required
                 value={user.clientId}
                 name="clientId"
                 onChange={handleChange}/>
                  {errors && (
                ...
                <TextField 
                 inputProps={ariaLabel} fullWidth
                  variant="filled"  label="EngineerId"
                   required
            ...
            <Box mb={3}> 
            <Tooltip open={open} onClose={handleClose} onOpen={handleOpen} title="click submit to book complaint">
                <Button variant="contained"  type="submit" fullWidth >Submit</Button></Tooltip>
                </Box>
